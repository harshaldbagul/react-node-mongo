...
const router = Router();
const getProductCollection = () => db.getClient().db('shops').collection('products');
const products = [
  {
    _id: 'fasdlk1j',
...
router.get('/', (req, res, next) => {
  ...
  const pageSize = Number(req.query.pageSize != undefined ? req.query.pageSize : 2);
  const page = Number(req.query.page != undefined ? req.query.page : 0);

  let products = (req.query.pageSize != undefined || req.query.page != undefined) ? getProductCollection().find({}).sort({price:1}).skip(page * pageSize).limit(pageSize) : getProductCollection().find({}).sort({price:1});
  const temp = []
...
router.get('/:id', (req, res, next) => {
  getProductCollection().findOne({ _id: new ObjectId(req.params.id) })
    .then(productDoc => {
...
router.post('', (req, res, next) => {
  const newProduct = {
    ...
    image: req.body.image ? req.body.image : 'http://localhost:3100/images/product-purse.jpg'
  };

  getProductCollection().insertOne(newProduct).then((productDoc) => {
    res.status(201).json({ message: 'Product added', productId: productDoc._id });
  }).catch(err => {
    console.log(err);
    res.status(500).json({ error: 'Unexpected error' });
  });
...
router.patch('/:id', (req, res, next) => {
  ...
  getProductCollection().updateOne({ _id: new ObjectId(req.params.id) }, { $set: updatedProduct })
    .then(productDoc => {
...
router.delete('/:id', (req, res, next) => {
  getProductCollection().deleteOne({ _id: new ObjectId(req.params.id) })
    .then(productDoc => {
      res.status(200).json({ message: 'Product deleted successfully', productId: productDoc.id })
    }).catch(err => {
      console.log(err);
      res.status(500).json({ error: 'Unexpected error' });
    })
